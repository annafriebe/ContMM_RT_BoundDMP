...
def getSwitchAndWakeupDataForCPU(filePath, CPU, microSeconds = False):
    ...
    with open(filePath) as f:
        ...
        for i in range(25, nLines-1):
            line = lines[i].split()
            indexOffset = 0
            while (not line[1+indexOffset].startswith('[')):
                indexOffset += 1
            event = line[3+indexOffset]
            cpu = line[1+indexOffset]
            if (cpu == CPU):
                if event == 'sched_switch:':
                    time = timeInMicroOrNano(line[2+indexOffset], microSeconds)
                    inProcess = line[8+2*indexOffset]
                    outProcess = line[4+indexOffset]
                    timingData.append([0, time, inProcess, outProcess])
                if event == 'sched_wakeup:': 
                    time = timeInMicroOrNano(line[2+indexOffset], microSeconds)
                    process = line[4+indexOffset]
